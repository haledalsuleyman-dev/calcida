...
export function AfterTaxIncomeCalculator() {
  // Income Inputs
  ...
  // Pre-tax Deductions
  const [preTaxDeductions, setPreTaxDeductions] = useState(0); // 401k, HSA, etc.
  ...
  const result = useMemo<AfterTaxResult>(() => {
    ...
    // These reduce taxable income for Federal and usually State, but NOT for FICA (usually).
    // Note: 401k is FICA taxable, but exempt from Federal/State income tax.
    // Health insurance premiums are often exempt from FICA too.
    // For simplicity, we will assume "Pre-tax Deductions" here are 401k-style (FICA taxable, Income tax exempt).
    ...
    // FICA is usually based on Gross Pay (before 401k), but after health insurance.
    // We will use Gross Annual for FICA base to be conservative/standard.
    ...
    const netPayAnnual = Math.max(0, annualGross - totalTaxAnnual - preTaxDeductions);
  ...
  }, [incomeType, inputValue, hoursPerWeek, weeksPerYear, bonus, preTaxDeductions, filingStatus, state, federalTaxEnabled, stateTaxEnabled, ficaEnabled]);
  ...
  const chartData = [
    ...
    { name: 'Deductions', value: result.preTaxDeductions },
    ...
    <div className="grid gap-8 lg:grid-cols-12">
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="preTaxDeductions">Pre-tax Deductions (401k, etc.)</Label>
              <div className="relative">
                <span className="absolute left-3 top-2.5 text-gray-500">$</span>
                <Input
                    id="preTaxDeductions"
              ...
              <p className="text-xs text-gray-500 mt-1">Pre-tax deductions reduce taxable income.</p>
            </div>
          </div>
      ...
      <div className="lg:col-span-8 space-y-8">
        ...
        <div ref={resultCardRef} className="bg-blue-50 p-8 rounded-lg border border-blue-100 text-center shadow-sm">
          <h2 className="text-xl font-semibold text-blue-900 mb-2">Estimated After-Tax Income</h2>
          <div className="text-5xl font-bold text-blue-700 mb-2">
            {formatCurrency(result.netPayAnnual)}
          </div>
          ...
          <div className="flex justify-center gap-4">
             <Button 
                ...
                onClick={() => {
                    navigator.clipboard.writeText(`My estimated after-tax income is ${formatCurrency(result.netPayAnnual)} per year.`);
        ...
        <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                <h3 className="text-lg font-semibold mb-6">Take-Home Pay Breakdown</h3>
                <div className="space-y-4">
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                <h3 className="text-lg font-semibold mb-4">Annual Tax Breakdown</h3>
                <div className="space-y-3">
                    ...
                    <div className="flex justify-between pt-2 font-bold text-lg mt-2 text-blue-700">
                        <span>Net Pay</span>
        ...
        <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
            ...
            <ul className="space-y-3 text-gray-700">
                <li className="flex items-start gap-2">
                    ...
                    <span>
                        Your effective tax rate is approximately <strong>{((result.totalTaxAnnual / result.annualGross) * 100).toFixed(1)}%</strong>.
                    </span>
                </li>
                <li className="flex items-start gap-2">
                    <span className="mt-1.5 w-1.5 h-1.5 bg-blue-500 rounded-full flex-shrink-0"></span>
                    <span>
                        You keep about <strong>{((result.netPayAnnual / result.annualGross) * 100).toFixed(1)} cents</strong> of every dollar you earn.
                    </span>
                </li>
                {result.preTaxDeductions > 0 && (
                    <li className="flex items-start gap-2">
                        <span className="mt-1.5 w-1.5 h-1.5 bg-blue-500 rounded-full flex-shrink-0"></span>
                        <span>
                            Your pre-tax contributions of <strong>{formatCurrency(result.preTaxDeductions)}</strong> lowered your taxable income, reducing your Federal/State taxes.
                        </span>
                    </li>
                )}
            </ul>
